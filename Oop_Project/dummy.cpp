...
void Engine::StartGame() {
	...
	mistake_texture.loadFromFile("./img/mistake_zero.png");
	mistake_sprite.setTexture(mistake_texture);	mistake_sprite.setScale(sf::Vector2f(1.45, 1.45));
	...
	// Load sound files
	if (!correctBuffer.loadFromFile("./aud/correct.mp3") ||
		!wrongBuffer.loadFromFile("./aud/wrong.mp3") ||
		!successBuffer.loadFromFile("./aud/success.mp3") ||
		!looseBuffer.loadFromFile("./aud/loose.mp3")) {
...
void Engine::AddLettertoWord(unsigned int key) {
	...
	if (letter_found) {
		...
		mistake_texture.loadFromFile(sf::String("./img/") + sf::String(mistakes_img[++mistakes]));
